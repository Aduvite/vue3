...
export class snake extends GameObject {
    constructor(ctx, gamemap) {
        ...
        this.direction = 1; //初始方向debug
        this.eps = 1e-1;
        this.speed = 8;//每秒钟走几格
        this.apple_cell = new cell(-1, -1);
        this.apple_img = new Image();
        this.apple_img.src = "https://bpic.51yuansu.com/pic3/cover/03/92/46/5c3443e78a382_610.jpg";//食物图片
        this.eating = false;
        this.tail_cell = null;
    }
    ...
    update_body() {
        ...
        if (d >= 0) {
            ...
            if (headi === this.apple_cell.i && headj === this.apple_cell.j) {
                this.eating = true;
                const cell_1 = this.cells[this.cells.length - 1];
                this.tail_cell = new cell(cell_1.i, cell_1.j);
                this.put_an_apple();
